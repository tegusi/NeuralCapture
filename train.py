...
from src.utils.config import get_configs,dump_configs
...
def train():
    ...
    f = os.path.join(output_dir, 'args.ini')
    dump_configs(f,args)

    # prepare data
    sum_logger = SummaryWriter(output_dir)
    ...
    for i in range(1, N_iters+1):
        ...
        # body parameters
        shape = model_emb.shape(idx)
        pose = model_emb.pose(idx)
        trans = model_emb.trans(idx)
        ...
        keypoints_2d = geometry.projection(keypoints, extrinsic, intrinsic)
        ...
        # render
        ret = render_image(intrinsic, extrinsic, keypoints, pose = pose, shape = shape, trans = trans, idx = idx, rays=batch_rays,
        ...
        if args.optimize_smpl:
            pose_loss = torch.norm((model_emb.body_poses.weight - ori_poses)[idx]) ** 2
            loss += 2 * pose_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        # save results
        if i % args.i_print == 0:
            diff = torch.norm(model_emb.body_poses.weight - ori_poses) ** 2
        ...
        if i % args.i_img == 0:
            ...
            with torch.no_grad():
                ...
                i_log = np.random.choice(test_frames)
                for i_test in test_frames:
